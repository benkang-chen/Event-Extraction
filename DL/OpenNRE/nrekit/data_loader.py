...
class JsonFileDataLoader(file_data_loader):
    # One batch contains batch_size instances.
    MODE_INSTANCE = 0
    # One batch contains batch_size bags, instances in which have the same entity pair (usually for testing).
    MODE_ENTPAIR_BAG = 1
    # One batch contains batch size bags, instances in which have the same relation fact. (usually for training).
    MODE_RELFACT_BAG = 2
    ...
    def __init__(self, file_name, word_vec_file_name, rel2id_file_name, mode,
                 shuffle=True, max_length=120, case_sensitive=False, reprocess=False, batch_size=160):
        """
        ...
        if reprocess or not self._load_preprocessed_file():
            ...
            # Eliminate case sensitive 排除大小写敏感问题
            ...
            # (head, tail) -> scope
            # {"/guid/9202a8c04000641f8000000000003b38#/guid/9202a8c04000641f8000000000003cfc": [0, 1]
            # 表示这个实体对的范围是第一条示例
            self.entpair2scope = {}
            # (head, tail, relation) -> scope
            self.relfact2scope = {}
            # 每行一个句子，值是每个单词的编号。
            ...
            for i in range(self.instance_tot):
                ...
                if cur_relfact != last_relfact:
                    ...
                    last_relfact_pos = i
                #  p1表示head在句子中的位置，p2表示tail在句子中的位置,使用find空格也算一个字符，是字母级别的位置
                ...
                if pos1 == -1 or pos2 == -1:
                    raise Exception("[ERROR] Position error, index = {}, sentence = {}, head = {}, tail = {}"
                                    .format(i, sentence, head, tail))
                if pos1 >= max_length:
                    pos1 = max_length - 1
                if pos2 >= max_length:
                    pos2 = max_length - 1
        ...
        # The number of relation facts, without NA.
        self.relfact_tot = 0
    ...
    def next_batch(self, batch_size):
        ...
        if self.mode == self.MODE_INSTANCE:
            ...
            # 如果最后一个batch的数据小于batch_size，补0，保证每个batch的大小一致
...
if __name__ == '__main__':
    ...
    # JsonFileDataLoader(os.path.join(dataset_dir, 'train.json'),
    #                    os.path.join(dataset_dir, 'word_vec.json'),
    #                    os.path.join(dataset_dir, 'rel2id.json'),
    #                    mode=JsonFileDataLoader.MODE_RELFACT_BAG,
    #                    shuffle=True)
